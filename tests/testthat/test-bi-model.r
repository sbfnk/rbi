...
test_that("lines can be inserted",
{
  ...
  expect_true(!is_empty(insert_lines(model, lines = "beta ~ normal()", at_beginning_of = "transition")))
  expect_true(!is_empty(insert_lines(model, lines = "beta ~ normal()", at_end_of = "transition")))
  expect_true(!is_empty(insert_lines(model, lines = "beta ~ normal()", before = "dummy")))
  expect_true(!is_empty(insert_lines(model, lines = "beta ~ normal()", after = "parameter")))
  expect_error(insert_lines(model, lines = "noise beta"))
  expect_error(insert_lines(model, lines = "noise beta", after=35))
  expect_error(insert_lines(model, lines = "beta ~ normal()", by = "transition"), "by")
})
...
test_that("lines can be removed",
{
  rem <- model
  rem[11] <- NULL
  expect_equal(length(get_block(remove_lines(model, "transition", only="N"), "transition")), 1)
...
test_that("block operations work",
{
  ...
  expect_equal(model[param_block[-c(1, length(param_block))]], get_block(model, "parameter"))
  expect_equal(get_block(add_block(model, "observation", "dummy"), "observation"), "dummy")
