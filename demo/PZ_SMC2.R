...
logweight <- bi_read_var(bi_object$result$outputfile, "logweight")
weight <- log2normw(logweight)
mu <- bi_read_var(bi_object$result$outputfile, "mu")
g1 <- qplot(x = mu, y = ..density.., weight = weight, geom = "histogram") + xlab(expression(mu))
sigma <- bi_read_var(bi_object$result$outputfile, "sigma")
