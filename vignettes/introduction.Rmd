...
Lastly, `write` writes a model to a file, and `clone` creates a new `bi_model` object which is an exact copy of the original one.
...
```{R}
SIRdata <- bi_generate_dataset(SIRmodel, end_time=16*7, noutputs=16, seed=123)
```

This simulates the model a single time from time 0 until time 16*7 (e.g., 16 weeks with a daily time step), producing 16 output (one a week). Note that we have specified a seed to make this document reproducible. Remove the `seed` argument or specify a different seed to simulate your own data set.
...
Let's sample from the prior of the SIR mooel:

```{R}
bi$run(client="sample", target="prior", nsamples=1000, end_time=16*7, noutputs=16)
```

This step calls **LibBi** to sample from the prior distribution of the previously specified model, generating 1000 samples and each time running the model for 16 * 7 = 112 time steps and writing 16 outputs (i.e., every 7 time steps). **LibBi** parses the model, creates C++ code, compiles it and run the model. If the model is run again, it should do so much quicker because it will will use the already compiled C++ code to run the model:

```{R}
bi$run()
```

Any call of `run` preserves options passed to the previous call of `run` and `libbi`, unless they are overwritten by arguments passed to `run` (e.g., passing a new `nsamples` argument). Let's have a closer look at the `bi` object:

```{R}
str(bi)
```

We can see it contains 12 fields: `client` is the client we passed (see LibBi manual for other clients, e.g. `filter`); `config` allows to specify an external configuration file; the `options` field contains all the options we passed to `bi$run`
...
This is a list of 11 objects, 10 representing each of the (noise/state) variables and parameters in the file, and one number `clock`, representing the time spent running the model in milliseconds.
...
Let's perform inference using Particle Markov-chain Metropolis Hastings (PMMH). The following command will generate 32 * 5,000 = 16,000 simulations and therefore may take a little while to run.

```{R}
bi$run(client="sample", target="posterior", nparticles=32, obs=SIRdata, sample_obs=TRUE, nsamples=1000)
```

This samples from the posterior distribution. Remember that options are preserved from previous runs, so we don't need to specify `nsamples`, `end_time` and `noutputs` again, unless we want to change them. The `nparticles` option specifies the number of particles, and the `sample_obs` option tells the `run` command that we also want observation samples from the posterior distribution of trajectories.
...
Let's get the results of the preceding `run` command:
...
Has the chain converged to the stationary distribution? Compare the marginal posterior distributions to the "correct" parameters used to generate the data set:

```{R}
str(dataset)
```

For more details on using **coda**, see the website of the [coda package](https://cran.r-project.org/package=coda). For more plotting functionality, see the `plot_libbi` function in [RBi.helpers](https://github.com/sbfnk/rbi.helpers).
