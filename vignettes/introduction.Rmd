...
Let's perform inference using Particle Markov-chain Metropolis Hastings (PMMH). The following command will generate 256,000 simulations and therefore may take a little while to run.

```{R}
bi$run(client="sample", target="posterior", nparticles=256, obs=SIRdata, sample_obs=TRUE)
```
...
Compare the marginal posterior distributions to the "correct" parameters used to generate the data set:
